...
def create_model():
    ...
    conv1_a      = Conv2D(filters=64, kernel_size=[3, 3], padding='SAME', activation=tf.nn.relu)(inputs)
    pool1_a      = MaxPool2D(padding='SAME')(conv1_a)
    pool3_flat_a = Flatten()(pool1_a)
    ...
    conv3_b      = Conv2D(filters=128, kernel_size=[3, 3], padding='SAME', activation=tf.nn.relu)(pool2_b)
    pool3_b      = MaxPool2D(padding='SAME')(conv3_b)
    pool3_flat_b = Flatten()(pool3_b)
